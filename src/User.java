...
public class User {
	...
	public Map<Integer, String> round1_KeySharing(List<User> U1, int t) {
		...
		for (User m : U1) {
			...
			String message = String.format("%d|%d|%s|%s", this.id, m.id, Nsk_shares.get(idx).toString(),
	...
	public Round2Output round2_MaskedInput(Map<Integer, String> P_m_n, List<User> U2, BigInteger tau, BigInteger delta,
		...
		for (int i = 0; i < gradientSize; i++) {
			BigInteger masked = localGradient.get(i).add(prg_beta.get(i)).add(pos.get(i)).subtract(neg.get(i))
					.mod(Crypto.Q);
			x_hat.add(masked);
		}
	    ...
	    Element x_i_Zr = Crypto.toZr(localGradient.get(0));  // فقط مؤلفه اول

	    Element A = Crypto.g.powZn(r_i).getImmutable();                                    // A = g^r
		...
		BigInteger phi_i = new BigInteger(64, new SecureRandom()).mod(Crypto.r);
		Element phi_i_Zr = Crypto.toZr(phi_i);
		Element C = Crypto.g.powZn(phi_i_Zr).getImmutable(); // C = g^φ
		Element D = A.powZn(phi_i_Zr).getImmutable();
	    ...
	    return new Round2Output(x_hat, A, B);  // فقط A و Bداریم
	}
		...
		for (User m : U3) {
			...
			if (P_m_n.containsKey(key)) {
				...
				if (decrypted != null && decrypted.split("\\|").length == 4) {
					...
					if (nsk != null)
						Nsk_shares.put(m.id, nsk);
					if (beta != null)
						beta_shares.put(m.id, beta);
				}
			}
		}
	...
	public boolean round4_Verification_withJPBC(Element A_agg, Element B_agg, BigInteger sigma_first) {
	    Element sum_x_Zr = Crypto.toZr(sigma_first);  // Σx mod r

	    Element e_A_h = Crypto.pairing.pairing(A_agg, Crypto.h);           // e(A, h) = e(g,h)^Σr
	    Element e_B_g = Crypto.pairing.pairing(B_agg, Crypto.g);           // e(B, g) = e(g,g)^{k·Σr + Σx}
	    Element e_g_g = Crypto.pairing.pairing(Crypto.g, Crypto.g);       // e(g,g)

	    Element right = e_B_g.mul(e_g_g.powZn(sum_x_Zr.duplicate().negate())).getImmutable();
	    
	    boolean result = e_A_h.isEqual(right);
	    System.out.println("User " + id + " Verification: " + result); // برای دیباگ
	    return result;
	}
